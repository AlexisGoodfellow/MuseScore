...
MenuItem* AppMenuModel::makeFileMenu()
{
    ...
    // projects are managed by the editude launcher and persisted via OT ops,
    // so these actions would either fail or silently diverge from the server.
    if (!qEnvironmentVariableIsSet("EDITUDE_SESSION_URL")) {
        fileItems << MenuItemList {
            makeMenuItem(PROJECT_NEW_COMMAND),
            makeMenuItem(PROJECT_OPEN_COMMAND),
            makeMenu(TranslatableString("appshell/menu/file", "Open &recent"), recentSubMenuItems, "menu-file-open",
                     openRecentEnabled),
